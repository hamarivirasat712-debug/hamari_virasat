  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none select-none overflow-hidden" aria-hidden="true">
        ...
        {/* ── Large OM (ॐ) watermark — safely behind the right image column only ── */}
        <div
          className="absolute top-1/2 right-[2%] -translate-y-[52%] text-[#2A1208] opacity-[0.22]"
          style={{ fontSize: 'clamp(160px, 24vw, 340px)', fontFamily: 'Georgia, serif', lineHeight: 1 }}
        >
          ॐ
        </div>

        {/* ── Mandala rings — top-RIGHT corner only (away from left-column text) ── */}
        <div className="absolute -top-20 -right-20 w-[560px] h-[560px] rounded-full border-[2.5px] border-[#2A1208]/55" />
        <div className="absolute -top-8 -right-8 w-[420px] h-[420px] rounded-full border-[1.5px] border-dashed border-[#5E2E14]/50" />
        <div className="absolute top-4 right-4 w-[300px] h-[300px] rounded-full border-2 border-[#2A1208]/48" />
        <div className="absolute top-16 right-16 w-[180px] h-[180px] rounded-full border border-[#2A1208]/52" />
        <div className="absolute top-[104px] right-[104px] w-[90px] h-[90px] rounded-full border border-[#5E2E14]/58" />
        <div
          className="absolute -top-28 -right-28 w-[640px] h-[640px] rounded-full"
          style={{ background: 'radial-gradient(circle, rgba(42,18,8,0.12) 0%, transparent 65%)' }}
        />

        {/* ── Mandala rings — bottom-LEFT corner only (away from content) ── */}
        <div className="absolute -bottom-24 -left-24 w-[480px] h-[480px] rounded-full border-[2.5px] border-[#2A1208]/52" />
        <div className="absolute -bottom-12 -left-12 w-[350px] h-[350px] rounded-full border-[1.5px] border-dashed border-[#5E2E14]/48" />
        <div className="absolute -bottom-2 -left-2 w-[240px] h-[240px] rounded-full border-2 border-[#2A1208]/46" />
        <div className="absolute bottom-16 left-16 w-[120px] h-[120px] rounded-full border border-[#2A1208]/50" />
        <div
          className="absolute -bottom-36 -left-36 w-[560px] h-[560px] rounded-full"
        ...
        {/* Smaller bell — second from left */}
        <div className="absolute top-1 left-[72px] text-[#2A1208] opacity-[0.50]">
          <TempleBell width={28} />
        </div>
        {/* Smaller bell — second from right */}
        <div className="absolute top-1 right-[72px] text-[#2A1208] opacity-[0.50]">
          <TempleBell width={28} />
        </div>
        {/* Bell far RIGHT */}
        ...
        {/* Large — bottom-RIGHT (right column, image area) */}
        <div className="absolute bottom-4 right-6 text-[#2A1208] opacity-[0.60]">
          <LotusFlower size={110} />
        </div>
        {/* Medium — bottom-LEFT corner (inside the bottom-left ring) */}
        <div className="absolute bottom-4 left-6 text-[#2A1208] opacity-[0.55]">
          <LotusFlower size={80} />
        </div>

        {/* ── Diyas — right column bottom area only, above scroll indicator ── */}
        <div className="absolute bottom-28 right-16 flex items-end gap-8 text-[#2A1208] opacity-[0.60]">
          <Diya width={28} />
          <Diya width={34} />
          <Diya width={28} />
        </div>
